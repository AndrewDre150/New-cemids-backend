...
def fetch_line_70_values(request):
    # Fetch next 70 sensor data entries (starting from the 71st entry)
    line2_70_data = SensorData1.objects.order_by('id')[1220:1270]

    # Prepare a response with the fetched data
    response_data = [
        {"timestamp": data.timestamp.strftime('%Y-%m-%d %H:%M:%S'), "co2": data.co2}
        for data in line2_70_data
    ]
...
def assign_daywise_values3(request):
    # Fetch next 70 sensor data entries
    line2_70_data = SensorData1.objects.order_by('id')[1220:1270]
    ...
    # Assign CO2 levels to respective days
    for idx, data in enumerate(line2_70_data):
        day_index = idx % 7  # Calculate the day index (0 for Mon, 1 for Tue, ..., 6 for Sun)
        day = days[day_index]
        day_values[day].append(data.co2)  # Ensure 'co2' matches your model field name
...
def fetch_linegragh2_70_values(request):
    # Fetch next 70 sensor data entries (starting from the 71st entry)
    linegraph_70_data = SensorData1.objects.order_by('id')[1150:1220]

    # Prepare a response with the fetched data
    response_data = [
        {"timestamp": data.timestamp.strftime('%Y-%m-%d %H:%M:%S'), "co2": data.co2}
        for data in linegraph_70_data
    ]
...
def assign_daywise_values4(request):
    # Fetch next 70 sensor data entries
    linegraph_70_data = SensorData1.objects.order_by('id')[1150:1220]
    ...
    # Assign CO2 levels to respective days
    for idx, data in enumerate(linegraph_70_data):
        day_index = idx % 7  # Calculate the day index (0 for Mon, 1 for Tue, ..., 6 for Sun)
        day = days[day_index]
        day_values[day].append(data.co2)  # Ensure 'co2' matches your model field name
...
def get_daily_co2_values(request):
    today = timezone.now()
    start_of_week = today - timedelta(days=today.weekday())  # Monday of the current week
    end_of_week = start_of_week + timedelta(days=7)  # End of the current week (next Monday)
    ...
    for i in range(7):
        day_start = start_of_week + timedelta(days=i)
        day_end = day_start + timedelta(days=1)
        values_for_day = list(SensorData1.objects.filter(timestamp__range=(day_start, day_end)).values_list('co2', flat=True))
        
        daily_values.append({
            "day": days_of_week[i],
            "values": values_for_day
        })

    return JsonResponse(daily_values, safe=False)
